...
public class SampleQueue implements TrackOutput {
  ...
  public long lastWriteToHapticTime;
  ...
  public final synchronized boolean seekTo(long timeUs, boolean allowTimeBeyondBuffer) {
    ...
    if(this.upstreamFormat.sampleMimeType.startsWith("audio")){
      android.util.Log.d(TAG, "abc seekTo: ");
    }
    int offset =
  ...
  public final int sampleData(
      ...
      throws IOException {
//    android.util.Log.d(TAG, "type: " + upstreamFormat);
    if(upstreamFormat != null && upstreamFormat.sampleMimeType.startsWith("audio")){
      //android.util.Log.d(TAG, "abc sampleData: ");
      //容器从input中读取数据，在容器类型为audio压缩数据时使用自定义的读取方式。
      return sampleDataQueue.sampleData(input, length, allowEndOfInput,true);
    }
    return sampleDataQueue.sampleData(input, length, allowEndOfInput);
  }

  public byte[] getWriteAllocationNode(int offset,int length){
    if(offset == 0){
      //android.util.Log.d(TAG, "abc getWriteAllocationNode: ");
      if(sampleDataQueue.writeAllocationNode.next != null){
        sampleDataQueue.writeAllocationNode = sampleDataQueue.writeAllocationNode.next;
      }
      sampleDataQueue.writeAllocationNode.initialize(sampleDataQueue.allocator.allocate(),
              new SampleDataQueue.AllocationNode(sampleDataQueue.writeAllocationNode.endPosition,64 * 1024));
    }
    return sampleDataQueue.writeAllocationNode.allocation.data;
  }
  ...
  private synchronized void commitSample(
    ...
    ProgressiveMediaPeriod.hapSample.largestQueuedTimestampUs = largestQueuedTimestampUs;
    ...
    for (int i = 0; i < length && timesUs[searchIndex] <= timeUs; i++) {
      if (!keyframe || (flags[searchIndex] & C.BUFFER_FLAG_KEY_FRAME) != 0) {
        ...
        if (timesUs[searchIndex] == timeUs) {
          android.util.Log.d(TAG, "abc findSampleBefore: ");
