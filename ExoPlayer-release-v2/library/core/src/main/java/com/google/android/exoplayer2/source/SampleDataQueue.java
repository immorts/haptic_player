...
/* package */ class SampleDataQueue {
  ...
  public long curHapBytesWritten;

  public long totalHapByteWritten;
  public long curHapBytesRead;
  ...
  public int sampleData(DataReader input, int length, boolean allowEndOfInput,boolean isAudio) throws IOException {
    length = preAppend(length);
//    if(length < len){
//      Log.d("TAG", "abc sampleData: " );
//    }
//    if(writeAllocationNode.startPosition == 65536){
//      Log.d("TAG", "abc sampleData: ");
//    }
//    if(writeAllocationNode.translateOffset(totalBytesWritten) == 0){
//      Log.d("TAG", "abc sampleData: ");
//    }
    int bytesAppended =
            input.read(
    ...
    ProgressiveMediaPeriod.hapSample.sampleDataQueue.totalBytesWritten += length;
    postAppend(bytesAppended);
    return bytesAppended;
  }
  ...
  private int preHapAppend(int length){
    if (writeAllocationNode.allocation == null) {
      writeAllocationNode.initialize(
              allocator.allocate(),
              new AllocationNode(writeAllocationNode.endPosition, allocationLength));
    }
    return min(length, (int) (writeAllocationNode.endPosition - totalHapByteWritten));
  }
   ...
   * @return The first {@link AllocationNode} that contains unread bytes after this method returns.
   */
  public static AllocationNode readData(
  ...
  public void writeData(byte[] arr,int length){
    int remaining = length;
    int arrIndex = 0;
    while(remaining > 0){
      length = preHapAppend(remaining);
      System.arraycopy(arr,arrIndex,writeAllocationNode.allocation.data,translateHapOffset(totalHapByteWritten),length);
      arrIndex += length;
      totalHapByteWritten += length;
      remaining -= length;
      if(totalHapByteWritten == writeAllocationNode.endPosition){
        writeAllocationNode = writeAllocationNode.next;
      }
    }
  }

  public int translateHapOffset(long totalHapByteWritten) {
    return (int) (totalHapByteWritten - writeAllocationNode.startPosition);
  }
