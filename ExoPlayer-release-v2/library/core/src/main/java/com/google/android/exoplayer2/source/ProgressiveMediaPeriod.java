...
/* package */public final class ProgressiveMediaPeriod
  ...
  private Runnable filterAudioRunnable;
  public Thread hapticRunnable;
  private final Handler handler;

  @Nullable private Callback callback;
  @Nullable private IcyHeaders icyHeaders;
  public SampleQueue[] sampleQueues;
  public static SampleQueue hapSample;
  public static SampleQueue filterHapSample;
  private TrackId[] sampleQueueTrackIds;
  private boolean sampleQueuesBuilt;
  public static boolean isHapEnough;
  ...
  /* package */ int readData(
    ...
    long largestQueuedTimestampUs = sampleQueues[1].getLargestQueuedTimestampUs();
    long largestReadTimestampUs = sampleQueues[1].getLargestReadTimestampUs();
    Log.d("TAG", "read  Time:" + largestReadTimestampUs);
    Log.d("TAG", "queue Time:" + largestQueuedTimestampUs);
  ...
  @Override
  public TrackOutput getHapSample() {
    return hapSample;
  }
  ...
  private TrackOutput prepareTrackOutput(TrackId id) {
    ...
    //Log.d("TAG", "prepareTrackOutput: " + allocator.allocate());
...
//    if(trackCount == 1){
//      sampleQueueTrackIds = Arrays.copyOf(this.sampleQueueTrackIds, trackCount + 2);
//      sampleQueueTrackIds[trackCount + 1] = id;
//    }
    ...
    if(trackCount == 1){
//      sampleQueues = Arrays.copyOf(this.sampleQueues, trackCount + 2);
//      sampleQueues[trackCount + 1] =
//              SampleQueue.createWithDrm(new DefaultAllocator(true, C.DEFAULT_BUFFER_SEGMENT_SIZE),drmSessionManager,drmEventDispatcher);
      //sampleQueues[trackCount + 1] = trackOutput;
      hapSample = SampleQueue.createWithoutDrm(new DefaultAllocator(true, C.DEFAULT_BUFFER_SEGMENT_SIZE));
      filterHapSample = SampleQueue.createWithoutDrm(new DefaultAllocator(true, C.DEFAULT_BUFFER_SEGMENT_SIZE));
      filterAudioRunnable = new FilterAudioRunnable();
      hapticRunnable = new HapticRunnable();
      new Thread(filterAudioRunnable).start();
      new Thread(hapticRunnable).start();
...
//    new Thread(filterAudioRunnable).start();
  ...
  private boolean seekInsideBufferUs(boolean[] trackIsAudioVideoFlags, long positionUs) {
    ...
    for (int i = 0; i < trackCount; i++) {
      ...
      if(i == 1){
        //Log.d("TAG", "seekInsideBufferUs: " + sampleQueue.absoluteFirstIndex + sampleQueue.readPosition);
        hapticIndexWhilePlay = sampleQueue.absoluteFirstIndex + sampleQueue.readPosition;
      }
  ...
  public final class SampleStreamImpl implements SampleStream {
    ...
    public void writeData(String aacPath) {
//      try {
//        FileOutputStream fos = new FileOutputStream(aacPath);
//        SampleDataQueue.AllocationNode node = ProgressiveMediaPeriod.this.sampleQueues[1].sampleDataQueue.readAllocationNode;
//        while(node.next != null){
//          fos.write(node.allocation.data);
//          node = node.next;
//        }
//        fos.close();
//      } catch (Exception e){
//      }
//      Log.d("TAG", "abc writeData: ");
    }
  ...
  public int hapticIndexWhilePlay = 0;
  public final class HapticRunnable extends Thread{
    ...
    @Override
    public void run() {
      audioPlayer = new AudioPlayer(44100, AudioFormat
              .CHANNEL_OUT_STEREO, AudioFormat.ENCODING_PCM_16BIT);
      audioPlayer.init();
      while (true){
        //当pcm容器中有数据时，一帧一帧地读取
        if(filterHapSample.sampleDataQueue.totalHapByteWritten - filterHapSample.sampleDataQueue.curHapBytesRead > 0){
          Log.d("TAG", "cur Index run: " + sampleQueues[1].absoluteFirstIndex);
          isHapEnough = true;
          if(hapticIndexWhilePlay > hapIndex){
            hapIndex = hapticIndexWhilePlay;
            filterHapSample.sampleDataQueue.curHapBytesRead = Mp4Extractor.hapSampleTable.hapOffsets[hapIndex];
          }
          int tmpSize = Mp4Extractor.hapSampleTable.hapSampleSize[hapIndex];
          byte[] tmpArr = new byte[tmpSize];
          SampleDataQueue.readData(filterHapSample.sampleDataQueue.readAllocationNode,filterHapSample.sampleDataQueue.curHapBytesRead,tmpArr,tmpSize);
//          if(ExoPlayerImplInternal.isBefore){
//            Arrays.fill(tmpArr, (byte) 0);
//          }
          audioPlayer.play(tmpArr,0,tmpSize);
          filterHapSample.sampleDataQueue.curHapBytesRead += tmpSize;
          hapIndex++;
        }
      }
    }
  ...
  final class FilterAudioRunnable extends Thread{

    MediaCodec mediaCodec;

//    FileOutputStream out;

//    private final String pcmPath = Environment.getExternalStorageDirectory().getPath() + "/audio.pcm";

    private int curHapticIndex;
...
//      }
      ...
      mediaFormat.setInteger("max-bitrate",127999);
      mediaFormat.setInteger("sample-rate",44100);
      mediaFormat.setInteger("track-id",2);
      mediaFormat.setString("mime","audio/mp4a-latm");
      mediaFormat.setInteger("profile",2);
      mediaFormat.setInteger("bitrate",127999);
      mediaFormat.setString("language","und");
      mediaFormat.setInteger("aac-profile",2);
      mediaFormat.setInteger("encoder-delay",2048);
      mediaFormat.setLong("durationUs",94156916);
      mediaFormat.setInteger("channel-count",2);
      mediaFormat.setInteger("encoder-padding",21);
      mediaFormat.setInteger("max-input-size",591);
      ByteBuffer allocate = ByteBuffer.allocate(2);
      allocate.put((byte) 18);allocate.put((byte) 16);
      allocate.position(0);
      ...
      // 输入
      ByteBuffer[] outputBuffers = mediaCodec.getOutputBuffers(); // 用于存放解码后的数据
      // 输出
      MediaCodec.BufferInfo info = new MediaCodec.BufferInfo(); // 用于描述解码得到的byte[]数据的相关信息
      int size = 0;
      int curIndex = 0;
      List<Byte> lst = new ArrayList<>();
      int sampleLen = Mp4Extractor.hapSampleTable.sizes.length;
      while(true) {
        //在aac的数据容器中剩余数据大小超过6s时
        if(hapSample.largestQueuedTimestampUs - hapSample.lastWriteToHapticTime > 6_000_000){
          hapSample.lastWriteToHapticTime = hapSample.largestQueuedTimestampUs;
          long curWritten = hapSample.sampleDataQueue.totalBytesWritten;
          Log.d("TAG", "test time abc 1 ProgressiveMediaPeriod: " + System.currentTimeMillis());
          //循环去解码这6s的数据
          while (hapSample.sampleDataQueue.curHapBytesWritten < curWritten && !Thread.interrupted()) {
            int inIndex = mediaCodec.dequeueInputBuffer(0);//设置等待解码时间，0为不等待，-1为一直等待，其他为时间单位
            if (inIndex >= 0) {
              Log.d("TAG", "run: " + sampleQueues[1].toString());
              //获得每一帧aac的大小，并读取并交给解码器。
              ByteBuffer buffer = inputBuffers[inIndex];
              int sampleSize = Mp4Extractor.hapSampleTable.sizes[curIndex];
              byte[] tmpArr = new byte[sampleSize];
              SampleDataQueue.readData(hapSample.sampleDataQueue.readAllocationNode, (int) hapSample.sampleDataQueue.curHapBytesWritten,tmpArr,sampleSize);
              hapSample.sampleDataQueue.curHapBytesWritten += sampleSize;
              buffer.put(tmpArr);
              buffer.flip();
              mediaCodec.queueInputBuffer(inIndex, 0, Mp4Extractor.hapSampleTable.sizes[curIndex], Mp4Extractor.hapSampleTable.timestampsUs[curIndex], 0); // 通知MediaDecode解码刚刚传入的数据
            }
            ...
            switch (outIndex) {
              ...
              default:
                ...
                buffer.get(outData);
                //获得解码之后pcm数据
                //filterHapSample.sampleDataQueue.sampleData(outData,info.size);
                for (byte outDatum : outData) {
                  lst.add(outDatum);
                }
                Mp4Extractor.hapSampleTable.hapSampleSize[curHapticIndex] = outData.length;
                if(curHapticIndex > 0){
                  Mp4Extractor.hapSampleTable.hapOffsets[curHapticIndex] = Mp4Extractor.hapSampleTable.hapOffsets[curHapticIndex - 1] + outData.length;
                } else {
                  Mp4Extractor.hapSampleTable.hapOffsets[curHapticIndex] = outData.length;
                }
                curHapticIndex++;
                //清空缓存
                buffer.clear();
          ...
          //在得到6s以上的数据时，将其交给算法处理
          Log.d("TAG", "test time abc 2 ProgressiveMediaPeriod: " + System.currentTimeMillis());
          ...
          for (int i = 0; i < len; i++) {
            douArr[i] = (double) bytes[i];
          }
          douArr = filterFromJNI(douArr,len);
          for (int i = 0; i < len; i++) {
            bytes[i] = (byte) douArr[i];
          }
          Log.d("TAG", "test time abc 3 ProgressiveMediaPeriod: " + System.currentTimeMillis());
          //写入pcm数据容器
          filterHapSample.sampleDataQueue.writeData(bytes,len);
          Log.d("TAG", "test time abc 4 ProgressiveMediaPeriod: " + System.currentTimeMillis());

        }
        if(curIndex == sampleLen){
          break;
        }
      }
      mediaCodec.stop();
  ...
  //native表示是加载的jni方法
  public native double[] filterFromJNI(double[] douArr,int length);

  static {
    //一定要记得加载so
    System.loadLibrary("exoplayer2");
  }
