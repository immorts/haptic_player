...
public final class DefaultExtractorInput implements ExtractorInput {
  ...
  @Override
  public int read(byte[] buffer, int offset, int length, byte[] hapticBuffer, boolean isAudio) throws IOException {
    return 0;
  }

  @Override
  public int read(byte[] buffer, int offset, int length, boolean isAudio) throws IOException {
    int bytesRead = readFromPeekBuffer(buffer, offset, length);
    if (bytesRead == 0) {
      bytesRead =
              readFromUpstream(
                      buffer, offset, length, /* bytesAlreadyRead= */ 0, /* allowEndOfInput= */ true,isAudio);
    }
    commitBytesRead(bytesRead);
    return bytesRead;
  }
  ...
  private int readFromUpstream(
          byte[] target, int offset, int length, int bytesAlreadyRead, boolean allowEndOfInput,boolean isAudio)
          throws IOException {
    if (Thread.interrupted()) {
      throw new InterruptedIOException();
    }
    byte[] writeAllocationNode = Mp4Extractor.hapSample.getWriteAllocationNode(offset + bytesAlreadyRead, length - bytesAlreadyRead);
