...
public final class Mp4Extractor implements Extractor, SeekMap {
  ...
  public static TrackOutput hapSample;
  public static TrackSampleTable hapSampleTable;
  ...
  private void processMoovAtom(ContainerAtom moov) throws ParserException {
    ...
    for (int i = 0; i < trackCount; i++) {
      ...
      if(i == 1){
        hapSample = extractorOutput.getHapSample();
        hapSampleTable = trackSampleTable;
        hapSampleTable.hapOffsets = new long[hapSampleTable.sizes.length];
        hapSampleTable.hapSampleSize = new int[hapSampleTable.sizes.length];
      }
  ...
  private int readSample(ExtractorInput input, PositionHolder positionHolder) throws IOException {
    ...
    //Log.d("TAG", "abc readSample: " + inputPosition + " " + sampleTrackIndex);
    if (sampleTrackIndex == 1) {
      Log.d("TAG", "abc readSample: ");
    }
    ...
    if (track.track.nalUnitLengthFieldLength != 0) {
      ...
      if (MimeTypes.AUDIO_AC4.equals(track.track.format.sampleMimeType)) {
        if (sampleBytesWritten == 0) {
          ...
          //hapSample.sampleData(scratch, Ac4Util.SAMPLE_HEADER_SIZE);
      ...
      while (sampleBytesWritten < sampleSize) {
        //完成了对文件头中信息的解析之后，播放器开始从原始资源中读取音视频压缩数据
