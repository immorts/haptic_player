...
public final class FileDataSource extends BaseDataSource {
  ...
  public int read(byte[] buffer, int offset, int length, boolean isAudio) throws IOException {
    if (length == 0) {
      ...
      int bytesRead;
      try {
        int size = (int) min(bytesRemaining, length);
        byte[] tmp = new byte[size];
        bytesRead = castNonNull(file).read(tmp, 0, size);
        System.arraycopy(tmp,0,buffer,offset,size);
        //    System.arraycopy(tmp,0,hapticBuffer,offset,size);
        //bytesRead = castNonNull(file).read(buffer, offset, (int) min(bytesRemaining, length));
      } catch (IOException e) {
        throw new FileDataSourceException(e, PlaybackException.ERROR_CODE_IO_UNSPECIFIED);
      }
  ...
  public int read(byte[] buffer, int offset, int length,byte[] hapticBuffer,boolean isAudio) throws FileDataSourceException {
    if (length == 0) {
      return 0;
    } else if (bytesRemaining == 0) {
      return C.RESULT_END_OF_INPUT;
